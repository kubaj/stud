...
**/
    ...
    *RootPtr = NULL;
    ...
    tBTNodePtr tmpNode = NULL;

    while (!SEmptyP(&Stack)) {
        tmpNode = STopPopP(&Stack);
        if (tmpNode->LPtr != NULL) SPushP(&Stack, tmpNode->LPtr);
        if (tmpNode->RPtr != NULL) SPushP(&Stack, tmpNode->RPtr);
        free(tmpNode);
    }
